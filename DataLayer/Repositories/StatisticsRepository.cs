...
";
        ...
        public List<StatisticQuestionCountByLocation> GetQuestionCountGroupedByLocation(DateTime dateFilter, int userId, NpgsqlConnection conn)
        {
            const string Sql = @"
SELECT
 COUNT(*) count
...
ORDER BY
    ...
    ,location_id
";
            List<StatisticQuestionCountByLocation> stats = new List<StatisticQuestionCountByLocation>();
            ...
            using (var cmd = new NpgsqlCommand(Sql, conn))
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        stats.Add(new StatisticQuestionCountByLocation
                        {
                            Count = int.Parse(reader["count"].ToString()),
                            QuestionId = int.Parse(reader["question_id"].ToString()),
                            QuestionText = reader["text"].ToString(),
                            CategoryId = int.Parse(reader["category_id"].ToString()),
                            CategoryName = reader["category_name"].ToString(),
                            LocationId = int.Parse(reader["location_id"].ToString()),
                        });
                    }
